...
def parse_quadrupole_file(fname, verbose=True):
    """
    Parse epw_quadrupole.out (streaming).
    """
    entries = []
    ik_ordered = []
    iq_ordered = []
...
def write_parsed_log(out_h5, parsed, qdat_coords, recip_vectors, kpath_coords,
    ...
    with open(log_path, "w") as fh:
        fh.write("Parsed EPW quadrupole summary log\n")
        fh.write("Generated by parse_epw_quadrupole.py\n")
        fh.write("===================================\n\n")
...
def build_and_write_h5(parsed, qdat_coords, out_h5, recip_vectors=None, tol=1e-6, compress=True, verbose=True):
    ...
    with h5py.File(out_h5, "w") as fh:
        ...
        meta.attrs["parsed_from"] = "epw_quadrupole.out"
        if fermi is not None:
...
# ---------------------------
def main():
    p = argparse.ArgumentParser(description="Parse EPW quadrupole output into HDF5")
    p.add_argument("--quad", required=True, help="epw_quadrupole.out file")
    p.add_argument("--qdat", required=True, help="q.dat (k-path fractional coords)")
    p.add_argument("--out", default="epw_quadrupole_parsed.h5", help="output HDF5 filename")
