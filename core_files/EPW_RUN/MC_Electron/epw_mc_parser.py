...
This script:
 - parses electron_inv_taucb_0.fmt (tau & energies)
 - parses electron_IBTEvel_sup_0.fmt (velocities in a.u., eig, weight)
...
Minimal addition: parses electron_m_effective.fmt (optional) and writes T_Mstar & Mstar_T datasets.
"""
...
def parse_effective_mass_file(fname="electron_m_effective.fmt"):
    """
    Minimal parser for electron_m_effective.fmt that extracts the temperature blocks
...
def main():
    ...
    p.add_argument("--tau", default="electron_inv_taucb_0.fmt")
    p.add_argument("--vel", default="electron_IBTEvel_sup_0.fmt")
    p.add_argument("--mstar", default="electron_m_effective.fmt")
