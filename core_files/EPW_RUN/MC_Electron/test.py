#!/usr/bin/env python3
"""
single_file_mc_fast_fixed_with_multiband_kcloud.py

Patched version: adds per-particle band tracking to support true multi-band
k-cloud outputs. Writes per-field folders kcloud/E_<field>/CB1.txt, CB2.txt, ...
CB1 corresponds to the lowest conduction band (LCB) used by the MC.

Changes (high level):
- build_channels_from_rows now stores ch_ib and ch_jb (initial and final band indices)
- selection kernel returns chosen *channel index* (not just iq)
- run_mc tracks part_band array and updates it on interband transitions
- run_mc returns final_part_iks and final_part_bands via energy_out
- main() writes per-field folders and per-band CB*.txt files with (ik, energy_eV, N_particles)

Note: This patch preserves backwards compatibility for purely intraband runs: only CB1.txt
will be created (and identical to previous kcloud files), and code paths for missing
interband channels remain safe.
"""
...
# ---------- compute_rates: identical to previous implementation (unchanged) ----------
...
def build_channels_from_rows(h5path, lcb, gamma_meV, T, kbt_multiplier, energy_tol_meV, nq, mode_flag, verbose=True):
    ...
    # new: store initial and final band indices per channel and partial Gamma components
    ch_ib = []; ch_jb = []
    ch_Gamma_abs_w = []; ch_Gamma_em_w = []
    for r in rows:
        g_abs_meV = float(r['g_abs_meV'])
        ib=int(r['ibnd']); jb=int(r['jbnd'])
        if ib < lcb or jb < lcb:
        ...
        ch_ib.append(ib); ch_jb.append(jb)
        ch_Gamma_abs_w.append(Gamma_abs * w_q)
        ch_Gamma_em_w.append(Gamma_em * w_q)
    ...
    ch_ib = np.array(ch_ib, dtype=np.int32)
    ch_jb = np.array(ch_jb, dtype=np.int32)
    ch_Gamma_abs_w = np.array(ch_Gamma_abs_w, dtype=np.float64)
    ch_Gamma_em_w = np.array(ch_Gamma_em_w, dtype=np.float64)
    ...
    ch_ib = ch_ib[order]; ch_jb = ch_jb[order]
    ch_Gamma_abs_w = ch_Gamma_abs_w[order]; ch_Gamma_em_w = ch_Gamma_em_w[order]
    ...
    return {
        ...
        'ch_ib': ch_ib, 'ch_jb': ch_jb,
        'ch_Gamma_abs_w': ch_Gamma_abs_w, 'ch_Gamma_em_w': ch_Gamma_em_w,
        'ch_start': ch_start, 'ch_len': ch_len, 'ch_cumsum': ch_cumsum
    }

# ---------- simple selection kernel (returns chosen channel index) ----------
if have_numba:
    @njit
...
def run_mc(channels, energies_band_eV, velocities_band, kpts_cart, ikq_map,
           ...
           energy_out=None, initial_band_idx=0, nbands_total=None):
    ...
    # --- NEW: per-particle band index ---
    # initial_band_idx is 0-based band index in the full energies array used for the MC
    part_bands = np.full((npart,), int(initial_band_idx), dtype=np.int32)

    # --- NEW: per-particle last-event tracker (0=none,1=abs,2=em)
    ...
    ch_ib     = channels['ch_ib'].astype(np.int32)
    ch_jb     = channels['ch_jb'].astype(np.int32)
    ch_Gamma_abs_w = channels.get('ch_Gamma_abs_w', None)
    ch_Gamma_em_w = channels.get('ch_Gamma_em_w', None)
    ...
    # Prepare cumulative hist arrays if nbands_total provided
    if nbands_total is None:
        nbands_total = int(initial_band_idx) + 1
    ...
    final_E = None
    final_part_iks = None
    final_part_bands = None
    final_part_last_event = None
    ...
    for t in range(steps):
        ...
        if np.any(to_scatter):
            ...
            # select channel indices (not iq)
            if have_numba:
                chosen_channels = select_channels_numba(part_iks, scatter_idx, rvals,
                                                       ch_start, ch_len, ch_cumsum)
            else:
                chosen_channels = select_channels_python(part_iks, scatter_idx, rvals,
                                                        ch_start, ch_len, ch_cumsum)

            for idx_local, p in enumerate(scatter_idx):
                ch_idx = int(chosen_channels[idx_local])
                if ch_idx < 0:
                    continue
                iq = int(ch_iq[ch_idx])
                old_ik = int(part_iks[p])
                # determine target ik (uses ikq_map if available)
                if ikq_map is not None:
                    ik_prime = int(ikq_map[old_ik, iq])
                else:
                    ik_prime = old_ik

                # determine new band (absolute 0-based)
                new_band = int(ch_jb[ch_idx]) - 1  # convert to 0-based

                # decide whether this channel resulted from abs or em
                ...
                Gsum = Ga + Ge
                if Gsum <= 0:
                    # ambiguous/elastic -- treat as none (0)
                    ev_type = 0
                else:
                    if Ga == 0.0:
                        ev_type = 2
                        ...
                        if np.random.random() < (Ga / Gsum):
                            ev_type = 1
                        else:
                            ev_type = 2

                # update cumulative hist counts (use new_band, ik_prime)
                ...
                # update particle: ik and band and last event
                part_iks[p] = ik_prime
    ...
    if energy_out is not None:
        ...
        energy_out['final_part_bands'] = final_part_bands
        energy_out['final_part_last_event'] = final_part_last_event
        energy_out['abs_hist'] = abs_hist
        energy_out['em_hist'] = em_hist
...
def main():
    ...
    p.add_argument('--gamma', type=float, default=15.0)                         # Lorentzian broadening (meV)
    p.add_argument('--T', type=float, default=77.0)                            # Temperature (K)
    p.add_argument('--kbt-mult', type=float, default=20.0)                      # Thermal window multiplier
    ...
    p.add_argument('--npart', type=int, default=20000)                          # Number of MC particles
    p.add_argument('--steps', type=int, default=20000)                          # Number of MC time-steps
    p.add_argument('--dt', type=float, default=0.03e-16)                        # Time-step (s)
    ...
    p.add_argument('--max-log-events', type=int, default=5000)                  # Max number of logged events
    ...
    for E_field in fields:
        ...
        vd, logged = run_mc(channels, energies_band, velocities_band, kpts_cart, ikq_map,
                            ...
                            energy_out=energy_out, initial_band_idx=band_idx)
        ...
        # --- Patch B (UPGRADED): write per-field folder with per-band CB files ---
        if ('final_part_iks' in energy_out) and (energy_out['final_part_iks'] is not None):
            ...
            final_bands = np.array(energy_out['final_part_bands'], dtype=np.int64)
            final_events = np.array(energy_out.get('final_part_last_event', np.zeros_like(final_iks)), dtype=np.int8)
            abs_hist = energy_out.get('abs_hist', None)
            em_hist = energy_out.get('em_hist', None)
            Nk_here = energies.shape[1]

            # Map: CB1 -> band_idx (0-based band index = band_idx), CB2 -> band_idx+1, ...
            base_band = band_idx  # 0-based index into energies matrix

            # create folder
            fldname = f"kcloud/E_{E_field:.3e}"
            os.makedirs(fldname, exist_ok=True)

            # determine which bands are actually populated (use final_bands)
            unique_bands = np.unique(final_bands)
            unique_bands = unique_bands[unique_bands >= base_band]

            # ensure CB1 is always included
            max_band_present = int(np.max(unique_bands)) if unique_bands.size>0 else base_band
            max_cb_index = max_band_present - base_band

            # for each CBn present (starting from base_band), write CBn files
            for cb_rel in range(0, max_cb_index+1):
                abs_band = base_band + cb_rel

                # counts per ik for total occupancy, last-event abs, last-event em, none
                counts_total = np.zeros((Nk_here,), dtype=np.int64)
                counts_abs_last = np.zeros((Nk_here,), dtype=np.int64)
                counts_em_last = np.zeros((Nk_here,), dtype=np.int64)
                counts_none_last = np.zeros((Nk_here,), dtype=np.int64)

                mask = (final_bands == abs_band)
                if np.any(mask):
                    bands_iks = final_iks[mask]
                    bands_events = final_events[mask]
                    counts_total = np.bincount(bands_iks, minlength=Nk_here)
                    # last-event breakdown
                    for ik_idx, ev in zip(bands_iks, bands_events):
                        if ev == 1:
                            counts_abs_last[int(ik_idx)] += 1
                        elif ev == 2:
                            counts_em_last[int(ik_idx)] += 1
                        else:
                            counts_none_last[int(ik_idx)] += 1
                else:
                    # no particles in this band at final step
                    pass

                cb_label = f"CB{cb_rel+1}"
                # total occupancy file (backwards compatible)
                fname = os.path.join(fldname, f"{cb_label}.txt")
                with open(fname, 'w') as fhk:
                    fhk.write("# ik_index   energy_eV   N_particles\n")
                    for ik in range(Nk_here):
                        e_val = float(energies[abs_band, ik]) if not np.isnan(energies[abs_band, ik]) else float('nan')
                        cnt = int(counts_total[ik])
                        fhk.write(f"{ik:6d}  {e_val:12.6f}  {cnt:8d}\n")
                print(f"[ok] Wrote k-cloud file: {fname}")

                # last-event files
                fname_abs = os.path.join(fldname, f"{cb_label}_abs.txt")
                fname_em  = os.path.join(fldname, f"{cb_label}_em.txt")
                fname_none = os.path.join(fldname, f"{cb_label}_none.txt")
                with open(fname_abs, 'w') as fhk:
                    fhk.write("# ik_index   energy_eV   N_particles_last_abs\n")
                    for ik in range(Nk_here):
                        e_val = float(energies[abs_band, ik]) if not np.isnan(energies[abs_band, ik]) else float('nan')
                        cnt = int(counts_abs_last[ik])
                        fhk.write(f"{ik:6d}  {e_val:12.6f}  {cnt:8d}\n")
                with open(fname_em, 'w') as fhk:
                    fhk.write("# ik_index   energy_eV   N_particles_last_em\n")
                    for ik in range(Nk_here):
                        e_val = float(energies[abs_band, ik]) if not np.isnan(energies[abs_band, ik]) else float('nan')
                        cnt = int(counts_em_last[ik])
                        fhk.write(f"{ik:6d}  {e_val:12.6f}  {cnt:8d}\n")
                with open(fname_none, 'w') as fhk:
                    fhk.write("# ik_index   energy_eV   N_particles_never_scattered\n")
                    for ik in range(Nk_here):
                        e_val = float(energies[abs_band, ik]) if not np.isnan(energies[abs_band, ik]) else float('nan')
                        cnt = int(counts_none_last[ik])
                        fhk.write(f"{ik:6d}  {e_val:12.6f}  {cnt:8d}\n")

                print(f"[ok] Wrote last-event files: {fname_abs}, {fname_em}, {fname_none}")

            # cumulative histograms (if available)
            if abs_hist is not None and em_hist is not None:
                for cb_rel in range(0, max_cb_index+1):
                    abs_band = base_band + cb_rel
                    fname_abh = os.path.join(fldname, f"CB{cb_rel+1}_abs_hist.txt")
                    fname_emh = os.path.join(fldname, f"CB{cb_rel+1}_em_hist.txt")
                    with open(fname_abh, 'w') as fh:
                        fh.write("# ik_index   total_abs_events\n")
                        for ik in range(Nk_here):
                            cnt = int(abs_hist[abs_band, ik]) if abs_band < abs_hist.shape[0] else 0
                            fh.write(f"{ik:6d}  {cnt:12d}\n")
                    with open(fname_emh, 'w') as fh:
                        fh.write("# ik_index   total_em_events\n")
                        for ik in range(Nk_here):
                            cnt = int(em_hist[abs_band, ik]) if abs_band < em_hist.shape[0] else 0
                            fh.write(f"{ik:6d}  {cnt:12d}\n")
                    print(f"[ok] Wrote hist files: {fname_abh}, {fname_emh}")
