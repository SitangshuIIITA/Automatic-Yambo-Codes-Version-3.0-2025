...
cd $DIR 
					...
					echo "=========================================================================="
				    	echo ">>> Running mobility for: holes at $ncarrier cm^-2 and $temps K:"
...
PATH3="$PATH1/H-Mobility_vs_T"


                            {   echo ""
                                echo "[$(date '+%Y-%m-%d %H:%M:%S')] QE Hole Mobility report:"
                                } >> "$DIR/EPW_work_report.txt"

# Create epw directory

mkdir -p $PATH1/EPW_plots/h-Mobility_vs_T



mkdir -p $PATH1/H-Mobility_vs_T
...
for i in "${!orbitals[@]}"; do
    idx=$((i+1))
    echo "proj($idx) = '${orbitals[$i]}'" >> hmob_dipole_sp.in
done
...
for ((i=0; i<${#labels[@]}-1; i++)); do
    echo "wdata($((i+3))) = '${labels[$i]} ${coords[$i]} ${labels[$((i+1))]} ${coords[$((i+1))]}'" >> hmob_dipole_sp.in
done

last_idx=$(( ${#labels[@]} + 1 ))
echo "wdata($((last_idx))) = 'dis_win_min = $dis_win_min'" >> hmob_dipole_sp.in
echo "wdata($((last_idx+1))) = 'dis_win_max = $dis_win_max'" >> hmob_dipole_sp.in
echo "wdata($((last_idx+2))) = 'dis_num_iter = $dis_num_iter'" >> hmob_dipole_sp.in
echo "/" >> hmob_dipole_sp.in



awk -v var="		             vme  = '"dipole"' " '/lpolar/{print;print var;next}1' 	hmob_dipole_sp.in > tmp && mv tmp hmob_dipole_sp.in


cp -r hmob_dipole_sp.in hmob_quadrupole.in
sed 's/system_2d = '"'dipole_sp'"'/system_2d = '"'quadrupole'"'/g' 				hmob_quadrupole.in > tmp && mv tmp hmob_quadrupole.in

cp -r hmob_quadrupole.in spec_decom_quadrupole.in
...
awk -v var="		       mob_nfreq  = 300 " '/lpolar/{print;print var;next}1' 	spec_decom_quadrupole.in > tmp && mv tmp spec_decom_quadrupole.in
			...
			for lr in 'dipole_sp' 'quadrupole'
				do
				    ...
				    echo ">>> Running mobility for: electron-$lr."
					    ...
					    # Run EPW
						    $MPIRUN_EPW -input hmob_$lr.in > hmob_$lr.out
						
								mkdir -p "h-Mobility_$lr"
    					
    					      				cp -r hmob_$lr.out ./"h-Mobility_$lr"
    					      				cp -r inv_*.fmt ./"h-Mobility_$lr"
    					      				cp -r m_effective.fmt ./"h-Mobility_$lr"
    					      				cp -r mobility_*.fmt ./"h-Mobility_$lr"
    					      				cp -r IBTE*_*.fmt ./"h-Mobility_$lr"
    					      
    					      				cp -r "h-Mobility_$lr" $PATH1/EPW_plots/h-Mobility_vs_T/.
    	...
    	mv _quadrupole.fmt quadrupole.fmt
				    			...
				    			echo ">>> Running spectral decomposition for: hole-quadrupole at 300 K."
...
cd $DIR
					...
					echo ">>> Ending mobility for: holes."
				    	echo "=========================================================================="
				
				
				
		
                         {  echo "    → Hole Mobility at "$temps" for dipole and dipole+quadrupole corrections."
