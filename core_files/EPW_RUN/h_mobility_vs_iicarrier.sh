...
cd $DIR 
					...
					echo "============================================================="
				    	echo ">>> Running Ionized and Grain Boundary Impurities Mobility For: Hole (quadrupole) at $ii_imp cm^-2 at $t K:"
				    

PATH2="$DIR/epw_$prefix"
PATH3="$DIR/epw_$prefix/H-mu_vs_ii"
PATH4="$DIR/epw_$prefix/gkkp-k-q-BZ"

                            {   echo ""
                                echo "[$(date '+%Y-%m-%d %H:%M:%S')] Hole Mobility report at Ionized and Grain Boundary Impurities:"
...
#mkdir -p $PATH2/EPW_plots/h-mu_vs_ii
...
echo "/" >> ${ii}_quad.in
				    ...
				    echo ">>> Running mobility for: hole-quadrupole at $ii cm^-2 at $t K"
				    


					mkdir -p II_H-mobility/"h-Mobility_${ii}"
					    ...
					    # Run EPW
						    $MPIRUN_EPW -input ${ii}_quad.in > ${ii}_quad.out
						    	...
						    	rm -rf restart_0.fmt restart_0.fmt.bak
    									...
    									cp -r ${ii}_quad.out ./II_H-mobility/"h-Mobility_${ii}"
    					      				cp -r inv_*.fmt ./II_H-mobility/"h-Mobility_${ii}"
    					      				cp -r m_effective.fmt ./II_H-mobility/"h-Mobility_${ii}"
    					      				cp -r mobility_*.fmt ./II_H-mobility/"h-Mobility_${ii}"
    					      				cp -r IBTE*_*.fmt ./II_H-mobility/"h-Mobility_${ii}"
    					      				
    					      				cp -r "II_H-mobility" $PATH2/EPW_plots
...
cd $DIR
				    	...
				    	echo ">>> Ending Ionized and Grain Boundary Impurities For Hole Mobility."
					echo "============================================================="
