...
source ../../core_files/QE_VAR/QE_variables.sh
					...
					echo "====================================================="
				    	echo ">>> Running & Plotting Hole Self-Energy."
			
cd $DIR

                            {   echo ""
                                echo "[$(date '+%Y-%m-%d %H:%M:%S')] Plotting of VB Self-Energy report:"
...
cd "$PATH4" || exit 1
	...
	scp -r "../../../core_files/EPW_RUN/plot_self_h_KGM.py" .
	INPUT="klist.dat"
...
python3 plot_self_h_KGM.py

scp -r "../../../core_files/EPW_RUN/plot_h_quadrupole_mode_bar.py" .
python3 plot_h_quadrupole_mode_bar.py


scp -r "../../../core_files/EPW_RUN/plot_h_scattering.py" .
python3 plot_h_scattering.py



					echo ">>> Ending & Plotting Hole Self-Energy. Check EPW_plots/self_energy Folders."
					echo "====================================================="


#scp -r *.png $DIR/epw_${prefix}/EPW_plots/.

   			 {  echo "    → Valence Self energies are plotted."
