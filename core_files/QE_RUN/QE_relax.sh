...
log_file="$DIR/check_folders.log"
    					...
    					echo "============================================================="
				    	echo ">>> Running QE Geometry Relaxation."
				    	

# Check if relax, scf_soc, or scf_nonsoc already exist
if [[ -d "$DIR/relax" || -d "$DIR/scf_soc" || -d "$DIR/scf_nonsoc" ]]; then
    # If folders exist, log the message and exit
    echo "One or more of the folders 'relax', 'scf_soc', or 'scf_nonsoc' already exist in $DIR." > "$log_file"
    echo "Skipping execution to avoid overwriting. Remove them manually to proceed." >> "$log_file"
    exit 1
else
    # If no folders exist, remove the log file if it exists
    if [[ -f "$log_file" ]]; then
        rm "$log_file"
    fi
fi
cd "$DIR" || exit 1
mkdir -p relax scf_soc scf_nonsoc
...
# Copy files to scf_soc and scf_nonsoc directories
...
mv 2 "$prefix.scf.in"
				    	...
				    	echo ">>> Ending QE Geometry Relaxation."
				    	echo "============================================================="
