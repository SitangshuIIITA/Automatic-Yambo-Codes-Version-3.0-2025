...
cd $DIR
	...
	mkdir BSE_folder
    	...
    	mkdir -p $DIR/graph_data/BSE_data_files

	cd BSE_folder
                      {
                            echo ""
                            echo "[$(date '+%Y-%m-%d %H:%M:%S')] Converged BSE report:"
                           } >> "$DIR/work_report.txt"
...
# Check if only one convergence parameter is given
if [ "${#ngsblk[@]}" -eq 1 ]; then
                       {
                          echo "Warning: BSE is not converged on energy cut-offs. Check again."
    ...
    char_bs_cpu=$bs_cpu  # Custom CPU assignment for unconverged run.
    ...
    #-----------------------------------------------Choise of QP gap: G0W0 or self-consistent GW----------------------#
                                    ...
                                    if [ "$BSE_gap" = "g0w0" ]; then
                                        ...
                                        scp -r "$PATH5/GW-$con_r$ryd/SAVE" ./.
                                        scp -r "$PATH5/GW-$con_r$ryd/output" ./.
                                            ...
                                            {
                                                        echo "    → QP gap is fixed from              : converged G0W0 = GW-$con_r$ryd."
...
#-----------------------------------------------------Running Yambo:----------------------------------------------------#
                                                    ...
                                                    mv o-output.alpha_q1_diago_bse $prefix.${char_ngsblk}$ryd.BSE_GW.o-output.alpha_q1_diago_bse
                                                    
                                                    scp -r $prefix.${char_ngsblk}$ryd.BSE_GW.o-output.alpha_q1_diago_bse $DIR/graph_data/BSE_data_files/.
                                                    
                                                    cd ..
                                                    
                                                    $YPP -J output -e s 1 2> out
                                                    
                                                    mv o-output.exc_qpt1_I_sorted $prefix.${char_ngsblk}$ryd.o-output.exc_qpt1_I_sorted
                                                    mv o-output.exc_qpt1_E_sorted $prefix.${char_ngsblk}$ryd.o-output.exc_qpt1_E_sorted
                                                    scp -r $prefix.${char_ngsblk}$ryd.o-output.exc_qpt1_E_sorted $DIR/graph_data/BSE_data_files/.
                                                    scp -r $prefix.${char_ngsblk}$ryd.o-output.exc_qpt1_I_sorted $DIR/graph_data/BSE_data_files/.
                                                    
                                            {
                                                        echo "    → Response block size               : NGsBlkXs = $char_ngsblk Ry"
                                                        echo "    → Response block size               : BSENGexx = $char_bseexx Ry"
                                                        echo "    → Response block size               : BSENGBlk = $char_bseng Ry"
                                                        echo "    → Polarization/Screening Bands      : Scheme = 1 | $bndr"
                                                        echo "    → BSE Transition Bands              : Scheme = $(cat insert_block)"
                                                        echo "    → See detailed gap report in $DIR/graph_data/BSE_data_files."
                                                        echo "    → Status                            : BSE calculations completed."
                                                        echo "End of converged BSE report."
                                                        echo ""
                                            } >> "$DIR/work_report.txt"
...
# Check if multiple convergence parameters are given
...
#-----------------------------------------------------Running Yambo:----------------------------------------------------#
...

                                            {
                                                        ...
                                                        echo "    → Polarization/Screening Bands      : Scheme = 1 | $bndr"
                                                        echo "    → BSE Transition Bands              : Scheme = $(cat insert_block)"
                                                        echo "    → Status                            : "$i"-th BSE calculations completed."
                                                        echo "    End of "$i"-th converged BSE report."
                                                        echo ""
                                                    } >> "$DIR/work_report.txt"
