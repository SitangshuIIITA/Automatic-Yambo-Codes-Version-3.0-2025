...
cd $DIR
        ...
        PATH3=$DIR/BSE_folder/BSE_Temperature
        PATH4=$DIR/GW_folder/GW_selfC
        PATH6=$DIR/GW_folder/GW_selfC
        
        
        mkdir -p $PATH2/BSE_Temperature
        mkdir -p $DIR/graph_data/BSE_Temperature_Data

        cd $PATH3
        
                          {
                            echo ""
                            echo "[$(date '+%Y-%m-%d %H:%M:%S')] BSE-Tempreature calculation report:"
                           } >> "$DIR/work_report.txt"
...
\"E W\" |\"+\" |\"1\" |\"../../../ELPH/QP-${line}${Kelvin}/qp-${line}${Kelvin}/ndb.QP\" |
                                               ...
                                               rm -rf "$prefix.BSE-${line}$Kelvin.diago.in"
                                               $YAMBO_PH 2> out
