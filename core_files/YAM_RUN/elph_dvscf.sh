...
cd $DIR
    ...
    PATH13=$DIR/dvscf
                            ...
                            {   echo ""
                                echo "[$(date '+%Y-%m-%d %H:%M:%S')] Computation of dV(q)/d\xi and phonon dynamical matrix elements for el-ph interaction report:"
                                } >> "$DIR/work_report.txt"
        ...
        STARTPWD=$DIR
			...
			mkdir phonon
				...
				echo "$K_GRID" >> $DYN0_FILE
                            ...
                            {
                            echo "    → Matrix elements written."
                            echo "[$(date '+%Y-%m-%d %H:%M:%S')] End of matrix report."
                            echo ""
                           } >> "$DIR/work_report.txt"
