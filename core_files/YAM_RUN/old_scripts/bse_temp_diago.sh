...
#       # 1. BSE spectra using diagonalization solver is computed at different temperatures. 
	...
	#-----------------------------------------------BSE + GW + phonon Calculation: diago solver------------------------------#
						...
						scp -r $DIR/database/output ./BSE-${line}$Kelvin/.
		...
		#----------------------------GW + temperature ELPH data merging-----------------------------------------------------#
                                               ...
                                               yambo_ph
                                               yambo_ph -r -rw -X s -optics b -kernel sex -Ksolver d -V all -F $prefix.BSE-${line}$Kelvin.diago.in
		
                #-----------------------------------------------COULOMB cut-off & RIM---------------------------------------------#

                                        sed -i 's/rimw_type= "default"/rimw_type= "'"$rimw_type"'"/g'                   $prefix.BSE-${line}$Kelvin.diago.in
                                        sed -i 's/RandGvecW= 1               RL/RandGvecW= '"$randgvecw $rl"'/g'        $prefix.BSE-${line}$Kelvin.diago.in
                                        sed -i 's/RandQpts=0/RandQpts='"$randqpts"'/g'                                  $prefix.BSE-${line}$Kelvin.diago.in
                                        sed -i 's/RandGvec= 1                RL/RandGvec= '"$randgvec $rl"'/g'          $prefix.BSE-${line}$Kelvin.diago.in
                                        sed -i 's/CUTGeo= "none"/CUTGeo= "'"$slab"'"/g'                                 $prefix.BSE-${line}$Kelvin.diago.in
               ...
               #----------------------------------------------FFT-GVecs and other Response size block-----------------------------#
                                                ...
                                                sed -i.bak -e '172d'                                                    $prefix.BSE-${line}$Kelvin.diago.in
                                                sed -i.bak "172i NGsBlkXs= $ngsblks"                                    $prefix.BSE-${line}$Kelvin.diago.in

                                                sed -i.bak -e '89d'                                                     $prefix.BSE-${line}$Kelvin.diago.in
                                                sed -i.bak "89i BSENGBlk= $bsengblk"                                    $prefix.BSE-${line}$Kelvin.diago.in

                                                sed -i.bak -e '83d'                                                     $prefix.BSE-${line}$Kelvin.diago.in
                                                sed -i.bak "83i BSENGexx= $bsengexx"                                    $prefix.BSE-${line}$Kelvin.diago.in
                ...
                #----------------------------In-plane electric field directions-------------------#             
                                                ...
                                                sed -i '142s/ 1.000000 | 0.000000 | 0.000000 |/ 1.000000 | 0.000000 | 0.000000 |/g'     $prefix.BSE-${line}$Kelvin.diago.in
                                                sed -i '182s/ 1.000000 | 0.000000 | 0.000000 |/ 1.000000 | 0.000000 | 0.000000 |/g'     $prefix.BSE-${line}$Kelvin.diago.in
		...
		#----------------------------Include Bose Temperature-----------------------------#
						...
						sed -i.bak -e '137d'                                                $prefix.BSE-${line}$Kelvin.diago.in
                                                sed -i.bak "137i 0.0500000 | 0.0500000 |         eV"                $prefix.BSE-${line}$Kelvin.diago.in

						sed -i.bak -e '175d'                                                $prefix.BSE-${line}$Kelvin.diago.in
                                                sed -i.bak "175i  0.01 | 0.01 |   eV"                 		    $prefix.BSE-${line}$Kelvin.diago.in
		...
		#---------------------Includes GW + temperature dependent band gap energies--------#
               	...
               	#------------------------Reading the k point limits, max valence band, min conduction band from r_setup file-------------#
                                                ...
                                                sed -i 's/$/|/' 9
                                                sed -i.bak '122s/.*//' $prefix.BSE-${line}$Kelvin.diago.in
                                                sed -i.bak '122 r 9' $prefix.BSE-${line}$Kelvin.diago.in
                                                sed -i.bak '122d' $prefix.BSE-${line}$Kelvin.diago.in
                ...
                #----------------------Includes photon energy range, W screening bands, writes exciton wavefunctions--------------------#
                                                ...
                                                sed -i '134s/0.00000 | 10.00000 |/0.00000 | 7.00000 |/g'                $prefix.BSE-${line}$Kelvin.diago.in
                                                sed -i 's/BEnSteps= 100/BEnSteps= 1000/g'                                $prefix.BSE-${line}$Kelvin.diago.in
                                                sed -i 's/#WRbsWF/WRbsWF/g'                                             $prefix.BSE-${line}$Kelvin.diago.in



                                                $MPIRUN_YAMBO yambo_ph -F $prefix.BSE-${line}$Kelvin.diago.in -J output -C Report
