...
#SBATCH --partition=standard
	...
	cd $PATH1
		...
		#------------------------------------------Quasi-particle energies calculation-----------------------------------------------------------------#		
					...
					sed -i 's/GDamping= 0.100000         eV/GDamping= 0.300000         eV/g'      	qp-${line}$Kelvin.in
					...
					#------------------------Reading the k point limits, max valence band, min conduction band from r_setup file-------------#
                                        ...
                                        sed -i 's/$/|/' 9
                                        sed '/^$/d' 9 > 10
                                        sed '44s/.*//' qp-${line}$Kelvin.in > new_file.txt
                                        sed '44 r 10' new_file.txt > 1new_file.txt
                                        sed '44d' 1new_file.txt > qp-${line}$Kelvin.in
		...
		#---------------------------------------------Spectral function calculation: Indirect gap---------------------------------------------------------#
					...
					#------------------------------COULOMB cut-off & RIM------------------------------#
                                        ...
                                        sed -i 's/GEnSteps= 100/GEnSteps= 1000/g'                              	sf-${line}$Kelvin.ind_gap.in
                                        sed -i '$a ExtendOut'                                                   sf-${line}$Kelvin.ind_gap.in

                                        sed -i.bak -e '44d' 							                        sf-${line}$Kelvin.ind_gap.in
                                        sed -i.bak "44i -1| 1|  eV" 						                    sf-${line}$Kelvin.ind_gap.in

                                        sed -i.bak -e '47d'                                                     sf-${line}$Kelvin.ind_gap.in
                                        sed -i.bak "47i 0.1| 0.1|  eV"                                          sf-${line}$Kelvin.ind_gap.in
					...
					sed -i 's/$/|/' 5
                                        ...
                                        sed '50s/.*//' sf-${line}$Kelvin.ind_gap.in > new_file.txt
                                        sed '50 r 5' new_file.txt > 1new_file.txt
                                        sed '50d' 1new_file.txt > sf-${line}$Kelvin.ind_gap.in
			...
			#---------------------------------------------Spectral function calculation: direct gap-----------------------------------------------------------#
					...
					yambo_ph -r -g g -p fan -c ep -V gen -F sf-${line}$Kelvin.dir_gap.in
                                        ...
                                        sed 's/$/|/' 1 > 2
                                        sed '1 s/./0.000000|0.000000|&/' 2 > 3
                                        sed '29s/.*//' sf-${line}$Kelvin.dir_gap.in > 4
                                        sed '29 r 3' 4 > 5
                                        sed '29d' 5 > 6
                                        ...
                                        sed -i 's/GEnSteps= 100/GEnSteps= 1000/g'                               sf-${line}$Kelvin.dir_gap.in
                                        sed -i '$a ExtendOut'                                                   sf-${line}$Kelvin.dir_gap.in

                                        sed -i.bak -e '44d'                                                     sf-${line}$Kelvin.dir_gap.in
                                        sed -i.bak "44i -1| 1|  eV"                                             sf-${line}$Kelvin.dir_gap.in

                                        sed -i.bak -e '47d'                                                     sf-${line}$Kelvin.dir_gap.in
                                        sed -i.bak "47i 0.1| 0.1|  eV"                                         sf-${line}$Kelvin.dir_gap.in
                                        ...
                                        sed '50s/.*//' sf-${line}$Kelvin.dir_gap.in > new_file.txt
                                        sed '50 r 5' new_file.txt > 1new_file.txt
                                        sed '50d' 1new_file.txt > sf-${line}$Kelvin.dir_gap.in
                                        rm -rf 1 2 3 4 5  new_file.txt 1new_file.txt *.bak
